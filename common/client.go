...
func QueryLastHeader(jrpcURl string) (*jsonrpc.Header, error) {
	var res jsonrpc.Header
	ctx := NewRpcCtx(jrpcURl, jsonrpc.Chain33_GetLastHeader, nil, &res)
...
func QueryBlockHash(jrpcURl string, height int64) (*jsonrpc.ReplyHash, error) {
	var res jsonrpc.ReplyHash
	params := types.ReqInt{
		Height: height,
	}
	ctx := NewRpcCtx(jrpcURl, jsonrpc.Chain33_GetBlockHash, params, &res)
	_, err := ctx.ReplyInterface()
	if err != nil {
		return nil, err
	}
...
func QueryIsSync(jrpcURl string) (bool, error) {
	var res bool
	ctx := NewRpcCtx(jrpcURl, jsonrpc.Para_IsSync, nil, &res)
	_, err := ctx.ReplyInterface()
	if err != nil {
		return false, err
	}
...
func QueryBalance(jrpcURl string, addrList []string) ([]*jsonrpc.Account, error) {
	...
	ctx := NewRpcCtx(jrpcURl, jsonrpc.Chain33_GetBalance, params, &res)
	_, err := ctx.ReplyInterface()
	if err != nil {
		return nil, err
	}
